...
class Distillation(Environment[State, specs.DiscreteArray, Observation]):
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[State, TimeStep[Observation]]:
        ...
        feed = jnp.array([0.0, 0.0, 0.2, 0.23, 0.0, 0.21, 0.0, 0.15, 0.0, 0.11], dtype=float)
        ...
        state = State(
            ...
            column_count=jnp.array(0, jnp.int32),
    ...
    def step(self, state: State, action: chex.Array) -> Tuple[State, TimeStep[Observation]]:
        ...
        reward = jnp.sum(jnp.nan_to_num(next_state.stream.value[:, state.column_count], nan=-45))
    ...
    def observation_spec(self) -> specs.Spec[Observation]:
        ...
        step_count = specs.DiscreteArray(
            self._max_steps+2, dtype=jnp.int32, name="step_count"
        )
    ...
    def _stream_table_update(self, state: State, column_state: ColumnState, action: chex.Array, iterator: chex.Array):
        ...
        column_cost = jnp.where((jnp.abs(jnp.nan_to_num(column_state.TAC) < 100.)) & (jnp.abs(jnp.nan_to_num(column_state.TAC)) > 0.), jnp.nan_to_num(-column_state.TAC / jnp.sum(column_state.F)), jnp.nan_to_num(-45./jnp.sum(column_state.F)))
